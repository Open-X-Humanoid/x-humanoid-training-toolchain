...
机器人控制接口以 replay_buffer_72demo.py 为准：
- 19维 state/action = 左臂7 + 左手1 + 右臂7 + 右手1 + 头部3
- 头部通过 ROS2 HeadController 独立控制
- 夹爪值除以 100 后再发送给 decompose_action
"""
...
CAMERA_VIEW_NAME = "head"  # 可视化窗口监控head / left / right
...
def _patch_replay_config(cfg_dict):
    """修正 xrocs 配置类型名（与 replay_buffer_72demo.py 一致）。"""
...
class WristCameraTopicRelay:
    """RealSense D405 发布 image_rect_raw，xrocs 订阅 image_raw，做 topic 转发。"""
...
# 初始化 ROS + XROCS
config_path = '/home/ubuntu/Documents/configuration.toml'  # TODO !!
...
# TODO
repo_id = "tianshu_dualArm_72_grab_and_flip_label_up_white_bag_barcode_upward_20260530"  # 修改1 # for init state
# TODO
...
print("xrocs-server listening on port", PORT)
...
while True:
    ...
    if len(pending_actions) > 0:
        ...
        # head_controller.move_head(DEFAULT_HEAD_JOINT3) # TODO !!!!!
