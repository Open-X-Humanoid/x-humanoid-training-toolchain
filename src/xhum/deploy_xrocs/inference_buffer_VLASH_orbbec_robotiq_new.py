...
# TODO !
pretrained_path = Path("/home/eai/Dev/wd/ckpt/pi05/multi_train_pi05_tianshu_73_new_white_upward_downward_unfreeze_ve_0611/checkpoints/008000/pretrained_model")
...
# 创建异步预测器
# model_action_steps=50 模型原始输出
async_predictor = VLASHAsyncPi0Predictor(
    policy=policy,
    model_action_steps=50,
    subsample_stride=1, # TODO ！！！ # 对生成的动作进行抽帧，可以做到加速；若subsample_stride=2 时间维 [::2] → 队列有效 25 步
    inference_trigger_threshold=30,  # inference_trigger_threshold 建议为 action_horizon(25) 的 20%-40%，即约 10~15
)
